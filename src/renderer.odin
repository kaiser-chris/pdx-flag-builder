    ...
    if exists {
        ...
        for variant in flag.Layers {
            ...
            case ^pdx.FlagLayerSub:
                // Request Sub Flag textures
                getFlagIdentifier(layer.Parent)
            }
        }
    }
...
renderFlag :: proc(flag: pdx.Flag, destination: rl.Rectangle) {
    if flag.Pattern.Name != "" {
        ...
        if ok {
            ...
            colorMappings := fillColorMapping(flag.Colors[:], flag, pdx.PATTERN_REPLACE_COLORS)
...
renderSubInstances :: proc(layer: ^pdx.FlagLayerSub, destination: rl.Rectangle) {
    flag, ok := state.Flags[layer.Parent]
    if !ok {
        return
    }
