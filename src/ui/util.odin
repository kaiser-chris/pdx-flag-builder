...
slider_u8 :: proc(ctx: ^mu.Context, val: ^u8, lo, hi: u8, fmtString := "%.0f") -> (res: mu.Result_Set) {
    mu.push_id(ctx, uintptr(val))

    @static tmp: mu.Real
    tmp = mu.Real(val^)
    res = mu.slider(ctx, &tmp, mu.Real(lo), mu.Real(hi), 0, fmtString, {.ALIGN_CENTER})
    val^ = u8(tmp)
    mu.pop_id(ctx)
    return
}

slider_f32 :: proc(ctx: ^mu.Context, val: ^f32, lo, hi: f32, fmtString := "%.0f") -> (res: mu.Result_Set) {
    mu.push_id(ctx, uintptr(val))
...
slider_i32 :: proc(ctx: ^mu.Context, val: ^i32, lo, hi: i32, fmtString := "%.0f") -> (res: mu.Result_Set) {
    mu.push_id(ctx, uintptr(val))
